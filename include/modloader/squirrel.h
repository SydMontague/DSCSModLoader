...
template<typename ParamType> int32_t pullParamValue(HSQUIRRELVM vm, ParamType& slot, size_t index)
{
...
// ************************* //
// TODO Needs specializing for each type a squirrel function can return
template<typename RetType> void pushReturnValue(HSQUIRRELVM vm, RetType& value)
{
    // requires specialization
}
...
template<typename RetType, typename... ParamTypes> SQInteger squirrelize(HSQUIRRELVM vm)
{
    // Get the function
